...
import { restoreLiveFromSnapshot, toSessionSnapshot, type RestoredSession } from "../platform/persistence/sessionSnapshot";
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const goNext = useCallback((force = false): "blocked" | "item" | "summary" => {
    ...
    if (prev.learnDrill) {
      if (prev.pathIndex >= prev.deck.length - 1) return "blocked";
      const pathIndex = prev.pathIndex + 1;
      const letter = prev.path[pathIndex];
      if (!letter) return "blocked";
      setSession(applyLetter({ ...prev, pathIndex }, letter));
      return "item";
    }
    if (!force && nextRequiresCheck({
      learnDrill: false,
      atFrontier: atFrontier(prev.pathIndex, prev.path.length),
      checkedCurrent: prev.checkedCurrent,
    })) {
      setSession({
        ...prev,
        writeNote: prev.kind === "quiz" ? t("check_first_quiz") : t("check_first_write"),
      });
      return "blocked";
    }
    if (prev.pathIndex < prev.path.length - 1) {
      const pathIndex = prev.pathIndex + 1;
      const letter = prev.path[pathIndex];
      if (!letter) return "blocked";
      setSession(applyLetter({ ...prev, pathIndex }, letter));
    ...
    const pool = remainingList(prev.deck, usedSet(prev));
    const letter = pickRandom(pool);
    if (!letter) {
      setSession({ ...prev, completed: true });
      return "summary";
    }
    const path = [...prev.path, letter];
    setSession(applyLetter({ ...prev, path, pathIndex: path.length - 1 }, letter));
    return "item";
  }, []);
