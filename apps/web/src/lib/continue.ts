...
export function learnedProgressForLast(
  last: SessionSnapshot | null | undefined,
  learned: Record<string, boolean>,
): { n: number; total: number } {
  const letters = lettersFor((last && last.setId) || "basic46");
  const script = last && last.script === "kata" ? "kata" : "hira";
  return { n: countLearned(learned, script, letters), total: letters.length };
}
