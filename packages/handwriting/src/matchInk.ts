...
import {
  isMatchProfileEnabled,
  profileAdd,
  profileNow,
  profileSetCandidates,
  profileSetTotal,
} from "./matchProfile";
...
export function matchInk(input: MatchInkInput): MatchResult {
  const tAll = isMatchProfileEnabled() ? profileNow() : 0;
  const bin = input.ink;
  ...
  profileSetCandidates(list.length);
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const precision = kata
      ? shiftedMetrics(userRaw, tmplD).precision
      : overlap.precision;
    const m = {
      ...overlap,
      precision,
  ...
  if (kata && rival && (HORIZ_SLANT.has(expectedRo) || VERT_SLANT.has(expectedRo)
    ...
    const tGeo = isMatchProfileEnabled() ? profileNow() : 0;
    ...
    if (isMatchProfileEnabled()) {
      profileAdd("geometryCalls", 1);
      profileAdd("geometryMs", profileNow() - tGeo);
    }
  ...
  if (ok && kata && !isCombo) {
    ...
    // Evaluate geometry for the conditional path without bypassing a failure.
    const tGeo = isMatchProfileEnabled() ? profileNow() : 0;
    geometryOk = kataGeometryOk(expectedRo, userRaw, bin, drawing);
    if (isMatchProfileEnabled()) {
      profileAdd("geometryCalls", 1);
  ...
  if (isMatchProfileEnabled()) profileSetTotal(profileNow() - tAll);
  return {
    status: ok ? "ok" : "bad",
